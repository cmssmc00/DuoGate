...
def _build_confidence_controller(
    ...
    """Mirror of the adaptation builder for the evaluation agent."""
...
class SimplifiedReActAgent(Agent):
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.partial_code_regex = r".*```(?:[a-zA-Z0-9_+-]+)?\n(.*)"
        self.full_code_regex = r"```(?:[a-zA-Z0-9_+-]+)?\n(.*?)```"
    ...
    def next_execution_inputs_and_cost(
        ...
        # ---- Confidence controller intercept (evaluation flow) -----------
        if self.confidence_controller is not None and code:
            try:
                ...
                # IMPORTANT: code AND content must move together so the
                # message history reflects what is actually executed.
